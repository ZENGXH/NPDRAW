...
def n_embenddings(nc, distribution='dmol'): #args.likelihood):
...
def dmol_loss(x, output, nc=3, nmix=NMIX, nbits=8):
    ...
    #logger.info('output: {}, x: {}', output.shape, x.shape)
    bits = 2. ** nbits
    ...
    # logger.info('loss: {}, cdf: {}, log_prob: {}', loss.shape, log_cdf.shape, logit_probs.shape)
...
def sample_from_dmol(x_mean, nc=3, nmix=NMIX, random_sample=False):
    ...
    # unpack values
    #logger.info('x_mean: {}', x_mean.shape)
    logit_probs = x_mean[:, :nmix]                                                                      # pi
    batch_size, nmix, H, W = logit_probs.size()
    means     = x_mean[:, nmix:(nc + 1) * nmix]
    #logger.info('means: {}', means.shape)
    means = means.view(batch_size, nmix, nc, H, W)                        # mean
